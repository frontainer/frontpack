...
module.exports = class FrontPack {
  ...
  _generatePath(prefix,name) {
    return [
      path.join(process.cwd(), `${prefix}/frontpack-${prefix}-${name}`),                   //  コマンド実行箇所のpreset内
      path.join(process.cwd(), `frontpack-${prefix}-${name}`),
      path.join(process.cwd(), `node_modules/frontpack-${prefix}-${name}`),
      path.join(process.cwd(), `node_modules/@frontpack/frontpack-${prefix}-${name}`), // コマンド実行箇所のnode_modules/@frontpack内
  ...
  export() {
    ...
    this._preset.forEach((preset) => {
      config = merge(config,util.require(this._generatePresetPath(preset))(this._option[preset],config));
    });
